...
SELECT
    ...
    SUM(ISNULL(t1.業務成本, 0)) AS 業務成本,
    SUM(ISNULL(t1.工程成本, 0)) AS 工程成本,
    SUM(ISNULL(t1.採購成本, 0)) AS 採購成本,
    SUM(ISNULL(t1.品保成本, 0)) AS 品保成本,
...
LEFT JOIN
    (
        SELECT 
            ...
            SUM(
                CASE WHEN j.departmentName = '業務' THEN j.WorkHour ELSE 0 END
            ) * 400 AS 業務成本,
            SUM(
                CASE WHEN j.departmentName = '工程' THEN j.WorkHour ELSE 0 END
            ) * 400 AS 工程成本,
            SUM(
                CASE WHEN j.departmentName = '採購' THEN j.WorkHour ELSE 0 END
            ) * 400 AS 採購成本,
            SUM(
                CASE WHEN j.departmentName = '品保' THEN j.WorkHour ELSE 0 END
            ) * 400 AS 品保成本
        FROM 
  ...
  try {
    ...
    const groupedData = data.reduce((acc, item) => {
      ...
      acc[year][quarter][month].push({
        ...
        業務成本: Number(item.業務成本).toFixed(0),
        工程成本: Number(item.工程成本).toFixed(0),
        採購成本: Number(item.採購成本).toFixed(0),
        品保成本: Number(item.品保成本).toFixed(0),
        ...
        訂單金額: Number(item.訂單金額).toFixed(0),
